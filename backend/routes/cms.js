...
router.get('/pages', async (req, res) => {
  try {
    const pages = await allAsync(
      'SELECT id, title, slug, meta_title, meta_description, created_at, updated_at FROM pages WHERE is_published = 1 ORDER BY title'
    );
    res.json(pages);
  } catch (error) {
...
router.post('/newsletter/subscribe', async (req, res) => {
  try {
    ...
    await runAsync(
      'INSERT INTO newsletter_subscriptions (email) VALUES (?) ON CONFLICT(email) DO UPDATE SET is_active = 1, subscribed_at = CURRENT_TIMESTAMP',
